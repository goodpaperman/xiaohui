...
void GetTextCallBack(HWND hWnd, UINT nMsg, DWORD dw, LRESULT res)
{
	TRACE("CallBack: 0x%x, nMsg: %d, dw: %d, res: %d.", hWnd, nMsg, dw, res); 
}
			...
			//!= (::GetWindowThreadProcessId (hwndCurr, NULL))) 
			{
				...
				if (::GetClassName(hwndCurr, lpClassName, 255))
				{
					...
					if (0 == m_strWndClass.CompareNoCase("EDIT"))
					{
						...
						::GetWindowText(hwndCurr, szText, 255); 
						TRACE("Password is: %s\n", szText); 
						//::SendMessageCallback(hwndCurr, WM_GETTEXT, 255, (LPARAM)szText, (SENDASYNCPROC)GetTextCallBack, (DWORD)this); 
