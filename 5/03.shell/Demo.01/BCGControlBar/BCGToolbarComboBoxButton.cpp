	...
	case CBN_SELENDOK:
		{
			...
			m_pWndCombo->GetLBText (m_iSelIndex, m_strText);
...
BOOL CBCGToolbarComboBoxButton::SelectItem (int iIndex)
{
	...
	m_pWndCombo->GetLBText (iIndex, m_strText);
	...
	CDC* pDC, const CRect& rect, BOOL bSelected)
{
	...
	CImageList images;
	images.Create (IDB_BCGBARRES_MENU_IMAGES, iImageWidth, 0, clrTransparent);

	IMAGEINFO imageInfo;
	images.GetImageInfo (0, &imageInfo);

	CRect rectImage = imageInfo.rcImage;

	CPoint pointTriangle (
		rectBtn.left + (rectBtn.Width () - rectImage.Width ()) / 2,
		rectBtn.top + (rectBtn.Height () - rectImage.Height ()) / 2);

	images.Draw (pDC, iImgIdArowDown, pointTriangle, ILD_NORMAL);
	return rect.Width ();
}
//********************************************************************************************
