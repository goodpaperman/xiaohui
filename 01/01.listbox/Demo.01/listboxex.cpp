...
void CListBoxEx::SelectNextItem(int nIndex)
{
    if(nIndex >= GetCount())
        nIndex = GetCount() - 1; 
                               ...
                               UINT nFlags )
{
        ...
        m_bAllowDrag )
   {
      switch ( nChar )
      {
         ...
         case VK_DOWN: // Alt + VK_DOWN does not work.
         case VK_RIGHT:
...
void CListBoxEx::BeginEditing( int iItem )
{
       ...
       (m_iEdited == -1) )
   {
      ...
      if ( m_dwEditStyle & LBEX_EDITBUTTON && m_pBrowseButton)
      {
                                      ...
                                      LPARAM lParam )
{
   ...
   switch ( lpEndEditInfo->uKind )
   {
      ...
      case IPEK_FOCUS:
         ...
         if ( (m_pEdit && lpEndEditInfo->hNewWnd == m_pEdit->m_hWnd) || 
              (m_pBrowseButton && lpEndEditInfo->hNewWnd == m_pBrowseButton->m_hWnd) )
         {
            bEndEdit = FALSE;
         }
         else
   ...
   switch ( iIndex )
   {
      ...
      case __BTN_DEL:
         ...
         if ( iSelected != -1 ) 
         {
             ...
             m_pListBoxEx->SelectNextItem(iSelected); 
         }
         break;
