...
BOOL CListBoxExDemoDlg::OnInitDialog()
{
	...
	// Add items to the listbox
        ...
        GetWindowsDirectory( szPath, MAX_PATH );
            m_ListBoxEx.InsertString( -1, szPath );
        GetSystemDirectory( szPath, MAX_PATH );
            m_ListBoxEx.InsertString( -1, szPath );
        GetTempPath( MAX_PATH, szPath );
            m_ListBoxEx.InsertString( -1, szPath );
        //m_ListBoxEx.SetItemData(2, LBEX_LASTITEM_MAGIC); 

         // Add the listbox buddy
         //m_ListBoxEx.SetEditStyle(0); 
         //m_ListBoxEx.AllowEditing(FALSE); 
         //m_ListBoxEx.AllowDrag(FALSE); 
