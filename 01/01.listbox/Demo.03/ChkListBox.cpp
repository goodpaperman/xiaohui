...
void CCheckControl::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	if( nItem >= m_nTop + m_nViewableRows  && pItem)
	{
		m_nTop++;
		ScrollWindow( 0, -m_pParent->GetLineHeight() );			
		SetScrollPos(SB_VERT, m_nTop, TRUE);
        point.y -= m_pParent->GetLineHeight(); 
	}
	...
	if( pItem )
	{
        ...
        ImageRect.top = (m_nScreenRow * m_pParent->GetLineHeight()) + PADDING; 
        ImageRect.bottom = ImageRect.top + m_pParent->GetLineHeight(); 
        //pDC->DrawFocusRect(ImageRect); 
...
// User Functions
/////////////////////////////////////////////////////////////////////////////
LISTITEM* CCheckList::AddString( CString csText, LISTITEM* pParentItem, 
                                int nCheckState, DWORD dwID, COLORREF crTextColor, 
                                COLORREF crTextHighColor, COLORREF crBgHighlightColor )
{
