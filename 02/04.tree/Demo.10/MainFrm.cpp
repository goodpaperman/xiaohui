	...
	CCreateContext* pContext)
{
	...
	if (!m_wndSplitter.CreateView(0, 0, RUNTIME_CLASS(CLeftView), CSize(150, 100), pContext) ||
		!m_wndSplitter.CreateView(0, 1, RUNTIME_CLASS(CGHCustomDrawView), CSize(100, 100), pContext))
	{
...
CGHCustomDrawView* CMainFrame::GetRightPane()
{
	CWnd* pWnd = m_wndSplitter.GetPane(0, 1);
	CGHCustomDrawView* pView = DYNAMIC_DOWNCAST(CGHCustomDrawView, pWnd);
	return pView;
}
...
void CMainFrame::OnUpdateViewStyles(CCmdUI* pCmdUI)
{
	...
	CGHCustomDrawView* pView = GetRightPane(); 
...
void CMainFrame::OnViewStyle(UINT nCommandID)
{
	...
	// if the right-hand pane has been created and is a CGHCustomDrawView,
...
BOOL CMainFrame::OnNotify(WPARAM wParam, LPARAM lParam, LRESULT* pResult) 
{
	...
	switch (pNmhdr->code)
	{
		case NM_CUSTOMDRAW:
		{
			...
			// from the left control (GGH)
			CLeftView *pView = GetLeftPane();
			if(pView->GetTreeCtrl().m_hWnd != pNmhdr->hwndFrom)
				break;
