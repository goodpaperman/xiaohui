...
// the usual window procedure...
  ...
  switch(uMsg)
  {
    ...
    case WM_RBUTTONDOWN:
      pClass=reinterpret_cast<CFlatPopupMenu *>(GetWindowLong(hWnd,GWL_USERDATA));
      pClass->OnRButtonDown(hWnd,LOWORD(lParam),HIWORD(lParam));
      break;
...
// search the menu items
  ...
  for(it=m_Items.begin();it!=m_Items.end();it++)
  {
    ...
    {
      if((pos=it->m_strName.find('&'))!=std::string::npos && 
          it->m_strName.length()>pos+1 && 
          c==(TCHAR)CharLower((LPTSTR)it->m_strName[pos+1]))
      {
...
// shift tracking if we're over another menu
  ...
  if((hPointWnd=WindowFromPoint(p))!=NULL && 
      GetClassName(hPointWnd,szClass,sizeof(szClass)/sizeof(TCHAR)) && 
      !lstrcmp(szClass,"fpmMenuClass"))
  {
