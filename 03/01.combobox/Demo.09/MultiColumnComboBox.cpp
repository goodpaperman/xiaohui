...
void CMultiColumnComboBox::FormatComboBox(UINT TotalColumn, UINT BoundColumn, UINT ShowColumn)
{
	...
	if (ComboBoxStyle != CBS_SIMPLE)
	{
		...
		for (UINT Index = 0, TotalColumnWidth = LEFTMARGIN_OF_DROPDOWNLIST + RIGHTMARGIN_OF_DROPDOWNLIST; Index < m_TotalColumn; Index++)
		...
		if (DropDownWindowPointer != this)		//Draw items in drop down list
		{
			...
			for (UINT ColumnIndex = 0; ColumnIndex < m_TotalColumn; ColumnIndex++)
			{
				...
				if (m_ColumnWidth[ColumnIndex] != 0)
				{
					...
					if ((ColumnIndex == m_TotalColumn - 1 || 
                        (ColumnIndex == m_TotalColumn - 2 && m_ColumnWidth[m_TotalColumn - 1] == 0)) && 
                        (GetWindowLong(DropDownWindowPointer->m_hWnd, GWL_STYLE) & WS_VSCROLL))
...
LRESULT CMultiColumnComboBox::OnSetText(WPARAM wParam, LPARAM lParam)
{
	...
	for (ShowColumnItemIndex = 0; ShowColumnItemIndex < BoundColumnItemCount && 
        m_ColumnItemList[m_BoundColumn].GetAt(m_ColumnItemList[m_BoundColumn].FindIndex(ShowColumnItemIndex)) != (LPCTSTR) lParam; ShowColumnItemIndex++);
...
//case insensitive:
	for (ShowColumnItemIndex = 0; ShowColumnItemIndex < BoundColumnItemCount && 
        (m_ColumnItemList[m_ShowColumn].GetAt(m_ColumnItemList[m_ShowColumn].FindIndex(ShowColumnItemIndex))).CompareNoCase((LPCTSTR) lParam) != 0; ShowColumnItemIndex++);
